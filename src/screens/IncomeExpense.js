...
const IncomeExpense = () => {
  ...
  const getIncomeExpenseData = (month, year, data) => {
    ...
    const arrDays = [];
    while (daysInMonth) {
      const day = daysInMonth < 10 ? `0${daysInMonth}` : daysInMonth;
      arrDays.push({
        date: `${day}.${month}.${year}`,
        income: 0,
        expense: 0,
  ...
  const getMontIncomeExpense = (direction) => {
    ...
    } else if (parseInt(currentDate.month) === 12) {
      month = '01';
      year = (parseInt(currentDate.year) + 1).toString();
    } else {
      month = parseInt(currentDate.month) + 1 < 10 ? `0${parseInt(currentDate.month) + 1}` : (parseInt(currentDate.month) + 1).toString();
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={{ flexDirection: 'row', backgroundColor: '#dddddd' }}>
        <Text style={[styles.item, { flex: 2.5, borderRightWidth: 0.5 }]}>Tarih</Text>
        <Text style={[styles.item, { flex: 2, borderRightWidth: 0.5 }]}>Gelir</Text>
        <Text style={[styles.item, { flex: 2, borderRightWidth: 0.5 }]}>Gider</Text>
        <Text style={[styles.item, { flex: 2, borderRightWidth: 0.5 }]}>Fark</Text>
